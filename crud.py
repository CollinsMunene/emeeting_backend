...
def get_user_by_id(db: Session, users_id: int):
    db_user = db.query(models.user_info).filter(
        models.user_info.users_id == users_id).first()
    return db_user
...
def get_all_participants_by_meeting_id(db: Session, meeting_id: int):
    participants = db.query(models.participant_voting_info).filter(
        models.participant_voting_info.meeting_id == meeting_id).all()
    all_part = []
    for i in range(len(participants)):
        user = get_user_by_id(db, users_id=participants[i].users_id)
        issue = get_meeting_issue_by_id(
            db, meeting_issues_id=participants[i].meeting_issues_id)
...
def get_participants_by_meeting_issue_user_id(db: Session, users_id: int, meeting_issues_id: int):
    participants = db.query(models.participant_voting_info).filter(
        models.participant_voting_info.meeting_issues_id == meeting_issues_id, models.participant_voting_info.users_id == users_id).first()
    return participants
...
def login(db: Session, userData: schemas.userInfo):
    print(userData)
    db_user = db.query(models.user_info).filter(
        models.user_info.email == userData.email).first()
    if db_user:
        passw = bcrypt.checkpw(userData.password.encode(
            'utf-8'), db_user.password.encode('utf-8'))
        print(passw)
        if passw:
            response = {"message": "success", "status": "OK", "data": db_user}
            return response
...
def create_meeting(db: Session, meetingData: schemas.meetingInfoBase):
    db_meeting = models.meeting_info(meeting_name=meetingData.meeting_name, meeting_agenda=meetingData.meeting_agenda,
                                     start_date_time=meetingData.start_date_time, end_date_time=meetingData.end_date_time)
    db.add(db_meeting)
    db.commit()
...
def update_meeting(db: Session, meetingData: schemas.meetingInfoBase, meeting_id: int):
    db_meetings = db.query(models.meeting_info).filter(
        models.meeting_info.meeting_id == meeting_id).first()
...
def create_meeting_issue(db: Session, meetingIssueData: schemas.meetingIssueInfo):
    db_meeting_issue = models.meeting_issues_info(
        issue_name=meetingIssueData.issue_name, meeting_id=meetingIssueData.meeting_id, votes=0)
...
def up_update_meeting_vote_by_meeting_id(db: Session, meeting_issues_id: int, users_id: int):
    db_meeting = db.query(models.meeting_issues_info).filter(
        models.meeting_issues_info.meeting_issues_id == meeting_issues_id).first()
    newVote = db_meeting.votes + 1
    db_meeting_updated = db.query(models.meeting_issues_info).filter(
        models.meeting_issues_info.meeting_issues_id == meeting_issues_id).update({models.meeting_issues_info.votes: newVote})
    db.commit()

    db_user = db.query(models.user_info).filter(
        models.user_info.users_id == users_id).first()
    
    db_issue_participation = models.participant_voting_info(
        users_id=db_user.users_id, meeting_id=db_meeting.meeting_id, meeting_issues_id=meeting_issues_id, vote="Yes", hasVoted=True)
    db.add(db_issue_participation)
    db.commit()
    db.refresh(db_issue_participation)

    response = {"message": "success",
                "status": "OK", "data": db_meeting_updated}
    return response
...
def down_update_meeting_vote_by_meeting_id(db: Session, meeting_issues_id: int, users_id: int):
    ...
    db_user = db.query(models.user_info).filter(
        models.user_info.users_id == users_id).first()
    db_issue_participation = models.participant_voting_info(
        users_id=db_user.users_id, meeting_id=db_meeting.meeting_id, meeting_issues_id=meeting_issues_id, vote="No", hasVoted=True)
    db.add(db_issue_participation)
    db.commit()
    db.refresh(db_issue_participation)
...
def upload_document_by_meeting_id(db: Session, meeting_id: str, document_name: str):
    db_document = models.document_info(
        meeting_id=meeting_id, document_name=document_name)
    db.add(db_document)
    db.commit()
